...
Water::Water(CRGB* leds, int startIndex, int endIndex, int pin, CHSV baseColor) :
    leds(leds), startIndex(startIndex), endIndex(endIndex), pin(pin) {
    pinMode(pin, INPUT_PULLUP);

    // Initialize colors with hue shift
    color1 = CHSV(baseColor.hue + 15, baseColor.sat, baseColor.val);
    color2 = CHSV(baseColor.hue - 15, baseColor.sat, baseColor.val);
    ...
    z = random8();
    speed = random8(1, 1);
    scale = random8(100, 100);
    noiseOffset = 0; 
    isOn = false; // Initialize isOn
     intensity = 0.0f;
}

float Water::smoothTransition(float startIntensity, float endIntensity, unsigned long startTime, int duration) {
...
void Water::updateColors() {
    EVERY_N_MILLISECONDS(40) {
        ...
        for (int i = startIndex; i <= endIndex; i++) {
            ...
            uint8_t noiseVal2 = inoise8(x + i * scale * 1, y + noiseOffset * 1, z * 1);
...
void Water::update(bool switchClosed) {
