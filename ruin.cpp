...
Ruin::Ruin(CRGB* leds, int startIndex, int endIndex, int pin, CHSV baseColor) :
    leds(leds), startIndex(startIndex), endIndex(endIndex), pin(pin) {
    pinMode(pin, INPUT_PULLUP);

    // Initialize colors with hue shift
    color1 = CHSV(baseColor.hue - 45, baseColor.sat, baseColor.val);
    color2 = CHSV(baseColor.hue + 45, baseColor.sat, baseColor.val);
...
float Ruin::smoothTransition(float startIntensity, float endIntensity, unsigned long startTime, int duration) {
...
void Ruin::updateColors() {
    EVERY_N_MILLISECONDS(60) {
        ...
        for (int i = startIndex; i <= endIndex; i++) {
            ...
            uint8_t noiseVal2 = inoise8(x + i * scale * 1.5, y + noiseOffset * 1.5, z * 1.1);
...
void Ruin::update(bool switchClosed) {
